...
# If we are *after* a merge (that is, HEAD is a commit with two parents), then `git mergetool` will never take any action.
# We must check out all the necessary files.
# Do that regardless!
...
for file in "${differing_files[@]}" ; do
  ...
  command="export BASE='$basefile'; export LOCAL='$leftfile'; export REMOTE='$rightfile'; export MERGED='$file'; $(git config --get mergetool.merge-java.cmd)"
  echo "$command"
